...
eGardener::eGardener(): memoryDist(),
                        wifi(WIFI_SERIAL_TX_PIN, WIFI_SERIAL_RX_PIN, WIFI_BAUDRATE),
                        wifi_ssid(), wifi_pwd(),
...
void eGardener::execute() {
    ...
    while (wifi.getStatus() != WiFiStatus::WL_CONNECTED && wifiTryCounter < WIFI_CONNECT_TRIES) {
...
void eGardener::setupMemoryDist() {
    uint16_t position = 0;
    // definir el tamaño no se si es medio inutil por como leo ahora, lo dejo por las dudas.

    memoryDist.insert(std::pair<std::string, std::pair<uint16_t, uint8_t>>("wifi", std::tuple<uint16_t, uint8_t>(position, sizeof(bool) + MAX_SSID_LENGTH + MAX_PWD_LENGTH + 2)));
    position += sizeof(bool) + MAX_SSID_LENGTH + MAX_PWD_LENGTH;

    memoryDist.insert(std::pair<std::string, std::pair<uint16_t, uint8_t>>("ls", std::tuple<uint16_t, uint8_t>(position, sizeof(bool) + sizeof(float) * 2)));
