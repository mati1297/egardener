...
Clock::Clock(PinName SDA, PinName SCL, uint8_t address): rtc(SDA, SCL), address(address << 1) {
    rtc.start();
    rtc.write(address | 0);
    rtc.write(0x07u);
    rtc.write(0x00u);
...
Time Clock::get() {
    ...
    return Time(seconds, minutes, hours, pm, mode_twelve, day, date, month, year);
}

// agregar twelve.
bool Clock::sync(WiFi& wifi, const std::string& timezone) {
    std::string url = "https://www.timeapi.io/api/Time/current/zone?timeZone=" + timezone;
    ...
    json.tokenIntegerValue(json.findChildIndexOf(json.findKeyIndex("day")), date);
    ...
    std::string dayOfWeek = json.tokenString(json.findChildIndexOf(json.findKeyIndex("dayOfWeek")));

    // hacer lo de day of the week.
    Time time(seconds, minutes, hours, false, false, Time::dayWordToDayNumber(dayOfWeek), date, month, year % 100);

    return set(time);
}

// pasar a aux esto. (esta en telegrambot tambien)
std::string Clock::getStringJson(const Json& json, int key) {
  int length = json.tokenLength(key);
  std::vector<char> array(length+1);

  const char * start = json.tokenAddress(key);
  memcpy(&array[0], start, length);

  return std::string(&array[0]);
}

Time::Time(uint8_t seconds, uint8_t minutes, uint8_t hours, bool pm, bool mode_twelve,
           uint8_t day, uint8_t date, uint8_t month, uint8_t year): 
           seconds(seconds), minutes(minutes), hours(hours), day(day), date(date),
           month(month), year(year), pm(pm), mode_twelve(mode_twelve) {
...
std::string Time::formatTime() const {
    if (mode_twelve) {
        ...
        snprintf(&buffer[0], BUFFER_SIZE_TIME_12 + 1, "%02u:%02u:%02u %s", hours, minutes, seconds, pm ? "PM" : "AM");
        ...
        snprintf(&buffer[0], BUFFER_SIZE_TIME_24 + 1, "%02u:%02u:%02u", hours, minutes, seconds);
...
std::string Time::formatDate() const {
    ...
    snprintf(&buffer[0], BUFFER_SIZE_DATE + 1, "%02u/%02u/20%02u", date, month, year);
